...
import { useMemo, useContext, createContext, useRef, useState, type ReactNode } from 'react'
...
export function Computers(props: any) {
  const { nodes: n, materials: m } = useGLTF('/computers_1-transformed.glb') as any
  const instances = useContext(context)
  
  // Safety check: if context isn't ready, render nothing
...
function ScreenText({ invert = false, x = 0, y = 1.2, resumeSection, ...props }: any) {
  ...
  const sectionId = resumeSection as ResumeSectionId | undefined
  return (
    <Screen
      {...props}
      tooltip={sectionId ? SECTION_LABELS[sectionId] : undefined}
      showTooltip={hovered && !!sectionId}
      onPointerOver={(e: any) => {
        e.stopPropagation()
        setHovered(true)
      }}
      onPointerOut={() => setHovered(false)}
      onClick={sectionId ? () => resume?.openResumeTo(sectionId) : undefined}
    >
...
function Leds({ instances }: any) {
  ...
  useFrame((state) => {
    ref.current.children.forEach((instance: any) => {
      ...
      // Instances in Merged handle color via the color attribute
  ...
  return (
    <group ref={ref}>
      <instances.Sphere position={[-0.41, 1.1, -2.21]} scale={0.005} />
      <instances.Sphere position={[0.59, 1.32, -2.22]} scale={0.005} />
      <instances.Sphere position={[1.77, 1.91, -1.17]} scale={0.005} />
      <instances.Sphere position={[2.44, 1.1, -0.79]} scale={0.005} />
      <instances.Sphere position={[4.87, 3.8, -0.1]} scale={0.005} />
      <instances.Sphere position={[1.93, 3.8, -3.69]} scale={0.005} />
      <instances.Sphere position={[-2.35, 3.8, -3.48]} scale={0.005} />
      <instances.Sphere position={[-4.71, 4.59, -1.81]} scale={0.005} />
      <instances.Sphere position={[-3.03, 2.85, 1.19]} scale={0.005} />
      <instances.Sphere position={[-1.21, 1.73, -1.49]} scale={0.005} />
    </group>
  )
}